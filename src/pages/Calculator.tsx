...
const Calculator: React.FC = () => {
	...
	const calculateBenefits = () => {
		...
		const tuitionNum = parseFloat(tuitionFees) || 0;
		...
		const tuitionCredit = tuitionNum * fedRate;
		const finalTaxOwed = Math.max(0, initialFedTax - tuitionCredit);
		const tuitionRefundEstimate = Math.max(0, withheldNum - finalTaxOwed);

		// --- 2. NEWCOMER ARRIVAL BONUS (RC151) ---
		// Added NL and PEI to the bonus tiers
		const bonuses: any = {
		...
		// --- 5. PROVINCIAL CREDITS (Updated for PEI/NL) ---
		let provName = "";
		let provAmt = 0;
		let oeptcRange = "";
		...
		// --- 6. TOTALS ---
		const baseTotal =
			nBonus + federalTotal + cwb + provAmt + renterCreditAmt + tuitionRefundEstimate;
		const totalLow = baseTotal + (province === "Ontario" ? 180 : 0);
		const totalHigh = baseTotal + (province === "Ontario" ? 450 : 0);

		const waMsg = encodeURIComponent(
			`Hi CMS Services! Estimate: $${totalLow.toFixed(0)}. Province: ${province}${isStudent ? " (Student)" : ""}. Tuition: $${tuitionNum}. I'd like to proceed with your $26.55 tax filing.`,
		);
			...
			{formData.isStudent && (
				<div className="student-fields animate-fade">
					<div className="form-group">
						<label>Tuition Fees Paid (T2202):</label>
						<input
							type="number"
							name="tuitionFees"
					...
					<div className="form-group">
						<label>Income Tax Deducted (T4 Box 22):</label>
						<input
							type="number"
							name="taxWithheld"
							placeholder="How much tax you already paid"
			...
			{results && (
				<div className="result animate-fade">
					...
					<div className="benefit-numbers">
						{results.tuitionRefundEstimate > 0 && (
							<p className="highlight-benefit">
								<span>Tuition Refund (Federal):</span>{" "}
								<span>${results.tuitionRefundEstimate.toFixed(2)}</span>
							</p>
						)}
						{results.nBonus > 0 && (
							<p>
					...
					<div className="tip-box">
						...
						{results.isStudent && results.tuitionRefundEstimate === 0
							? "Since you paid $0 in income tax, your tuition credits will 'Carry Forward' to save you thousands in taxes later!"
							: "Keep your T2202 and T4 slips handy for your $26.55 CMS filing!"}
					</div>
